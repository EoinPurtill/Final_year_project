...
class MonteCarloAgent:
    ...
    def __init__(self, checkers, colour, sim_time=5, c=1, b=0.1):
        print("init")
        self.colour = colour
    ...
    def static_concentric_val(self, game_state, king_coefficient=20):
        ...
        # Count the number of pieces of the player's colour,
        # and reward for kings. Subtract the opponents score.

        board, colour = game_state
        ...
        for i, row in enumerate(board.get_board()):
            for j, char in enumerate(row):
                if char == colour.lower():
                    play += 3 + concentric_coefficient(board, (i, j))
                if char == colour.upper():
                    play += king_coefficient

                if char == self.checkers.opponent(colour).lower():
                    oppo += 3 + concentric_coefficient(board, (i, j))
                if char == self.checkers.opponent(colour).upper():
    ...
    def best_child(self, node):
        print("child")
        enemy_turn = (node.game_state[1] != self.colour)
        values = {}
    ...
    def simulate(self, game_state):
        ...
        while True:
            ...
            if result is not None:
                ...
                if result == self.colour:
                    return 1, actions
                elif result == checkers_copy.opponent(self.colour):
                    return 0, actions
                elif result == "draw":
                    return .5, actions
                else:
